...
function CardCategorias({categorias}: CardCategoriasProps) {
    return (
      <div className="border-gray-300 border flex flex-col rounded-md overflow-hidden justify-between font-padrão w-[400px] hover:shadow-2xl">
        ...
        <Link to={`/editarCategoria/${categorias.id}`} className='w-32 rounded-md bg-all-azul-7 text-white hover:text-all-azul-7 hover:bg-all-cinza-7 font-brunoace flex items-center justify-center py-2'>
            <button>Editar</button>
          </Link>
          <Link to={`/deletarCategoria/${categorias.id}`} className='w-32 text-white rounded-md bg-red-400 hover:bg-red-700 font-brunoace flex items-center justify-center'>
